...
import 'components/quote_wrap.dart';
import 'components/section_wrap.dart';
import 'components/spoiler_wrap.dart';
import 'components/text_wrap.dart';

class DTextField extends StatelessWidget {
  final String msg;
  final bool darkText;

  DTextField(this.msg, {this.darkText = false});
  ...
  Widget build(BuildContext context) {
    ...
    TextSpan resolve(String msg, Map<String, bool> states) {
        ...
        if (start != null) {
          ...
          if (tag.isNotEmpty) {
            ...
            if (isFat) {
              ...
              spans.add(spanWrap(context, display, states, onTap: onTap));
              ...
              if ([
                ...
                if (split != null) {
                  ...
                  switch (key.toLowerCase()) {
                    case 'spoiler':
                      blocked = SpoilerWrap(
                          child: textWrap(resolve(between, states)));
                      break;
                    case 'code':
                      blocked = QuoteWrap(
                          child: textWrap(spanWrap(context, between, states)));
                      break;
                    case 'quote':
                      blocked =
                          QuoteWrap(child: textWrap(resolve(between, states)));
                      break;
                    case 'section':
                      blocked = SectionWrap(
                          child: textWrap(resolve(between, states)),
      ...
      for (String word in words) {
        ...
        if (rex.hasMatch(msg)) {
          for (Match wordMatch in rex.allMatches(msg)) {
            ...
            spans.add(spanWrap(context, match, states, onTap: onTap));
      ...
      void parseLink(String match, {bool insite = false}) {
        ...
        if (!insite) {
          ...
          onTap = () async {
            launch('https://${await db.host.value}$search');
          };
          try {
      ...
      return spanWrap(context, msg, states);
    }
    ...
    return textWrap(resolve(msg, states));
  }
}
