  ...
  void updateTitle() {
    ...
    title = () {
      if (follows != null && follows.contains(widget.provider.search.value)) {
        Follow follow = follows.follows.singleWhere(
            (follow) => follow.tags == widget.provider.search.value);
        int index = follows.follows.indexOf(follow);
        if (widget.provider.posts.value.isNotEmpty) {
          follows.updateLatest(index, widget.provider.posts.value.first);
        }
        if (pool != null) {
          follows.updatePoolName(index, pool);
        }
        return follow.title;
      }
      if (pool != null) {
